...
cm = !"f () { branch=$(git brn | cut -d '-' -f-2); msg=$1; git commit -m \"${branch}: ${msg}\"; unset branch; unset msg;}; f"
...
# commit / add all. Use at your own peril
cma = !sh -c 'git add . && git cm "$1" ' $1
# overwrite the file with the version at master. Again, peril.
mfile = !sh -c 'git checkout origin/master -- "$1" ' $1
# open the gui. This isn’t always configured by default. And your exe might be
# in another location. You can probably skip this one
gui = !sh -c '/usr/libexec/git-core/git-gui'
